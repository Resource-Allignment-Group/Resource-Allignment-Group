...
def test_classes(login_user):
    db = login_user.application.db
    classes = {eq._class for eq in db.get_all_equipment() if eq._class and eq._class !="None"}
...
def test_search_combined_fields(login_user):
    ...
    res = login_user.post("/search_equipment", json={"query": "5075E John Deere Test Aroostook"})
    data = res.get_json()
    names = [e["name"] for e in data["equip_list"]]
...
def test_search_score_threshold(login_user):
    ...
    res = login_user.post("/search_equipment", json={"query": "zzzzxyzzzzzxyxyxyzyzyyxy"})
    data = res.get_json()
