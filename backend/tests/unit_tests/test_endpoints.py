...
def test_all_user_endpoints(client):
    ...
    print(register_response.status_code, register_response.get_json())
    assert (
        register_response.status_code == 200 and register_response.get_json()["result"]
    )
    ...
    assert (
        change_role_response.status_code == 200
        and change_role_response.get_json()["result"]
    )
